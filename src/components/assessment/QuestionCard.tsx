...
export const QuestionCard = ({ question, value, onAnswer, onContinue }: Props) => {
  ...
  return (
    <motion.div
      ...
      className="w-full"
    >
      <h2 className="text-xl sm:text-2xl font-bold text-balance mb-1.5 text-foreground">
        {question.title}
      </h2>
      {question.subtext && (
        <p className="text-muted-foreground mb-2 text-sm">{question.subtext}</p>
      )}
          ...
          {question.options?.map((opt, i) => {
            ...
            return (
              <motion.button
                ...
                className={`group relative flex items-center gap-4 w-full text-left rounded-2xl border-2 px-4 py-3 transition-all duration-200 ${
                  selected
              ...
              >
                ...
                <span className="flex-1 font-semibold text-sm sm:text-base text-foreground">
                  {opt.label}
                </span>
                <span
      ...
      {question.type === "multi" && (
        <div className="mt-3">
          <button
            ...
            onClick={onContinue}
            className="group w-full inline-flex items-center justify-center gap-2 rounded-3xl gradient-primary text-primary-foreground font-semibold text-base px-6 py-3 shadow-glow hover:scale-[1.01] transition-transform disabled:opacity-50 disabled:hover:scale-100"
          >
            Continue
